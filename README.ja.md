...
- **ディスクイメージ版** — `Futtei-Kokatsu_*.dmg` (Apple Silicon 専用)

ダウンロードした `.dmg` を開き、中の `Futtei-Kokatsu.app` を「アプリケーション」フォルダにドラッグして下さい。未署名のためそのままでは開けず、 *ゴミ箱に捨てろ* と言われてしまいます(ひどい😭)。ターミナルから次のコマンドで検疫属性を外すと起動できるようになります。自己責任でどうぞ。
...
|---|---|
| Windows | 10 / 11 (64bit) |
| macOS | 11 (Big Sur) 以降 (Apple Silicon) |
